...
cdef class SpectralAnalysis:
    ...
    cpdef compute_spectral_flux(self, Parameters Pr, Grid Gr, PrognosticVariables PV, DiagnosticVariables DV):
        cdef:
            ...
            double complex [:,:] u_spec, v_spec, E_spec
            double [:,:] dp, dps_dx, dps_dy, dudx, dudy, dvdx, dvdy, uak, vak
        ...
        for k in range(nl):
            ...
            U_adv = np.add(np.multiply(PV.U.values[:,:,k],dudx), np.multiply(PV.V.values[:,:,k],dudy))
            V_adv = np.add(np.multiply(PV.U.values[:,:,k],dvdx), np.multiply(PV.V.values[:,:,k],dvdy))
            ...
            uak = np.multiply(dp, Gr.SphericalGrid.grdtospec(U_adv + U_vert_adv))
            vak = np.multiply(dp, Gr.SphericalGrid.grdtospec(V_adv + V_vert_adv))
            ...
            if k==nl:
                ...
                pressure_contribution = 0.5*Gr.SphericalGrid.grdtospec(np.add(np.multiply(PV.U.values[:,:,k],dps_dx),
                                                                              np.multiply(PV.V.values[:,:,k],dps_dy)))
            ...
            for i in range(0,np.amax(Gr.shtns_l)+1):
                self.KE_spec_flux_div[i,k] = np.sum(E_spec[np.logical_and(Gr.shtns_l>=i-0.5 , Gr.shtns_l<i+0.5)])
    ...
    cpdef compute_turbulence_spectrum(self, Parameters Pr, Grid Gr, PrognosticVariables PV):
        ...
        for k in range(nl):
            ...
            for i in range(0,np.amax(Gr.shtns_l)):
                self.KE_spectrum[i,k] += np.sum(np.real(KE_spectrum[Gr.shtns_l==i]), axis = 0)
                self.KE_Rot_spectrum[i,k] += np.sum(np.real(Vort2_spect.real[Gr.shtns_l==i]), axis = 0)
                self.KE_Div_spectrum[i,k] += np.sum(np.real(Div2_spect.real[Gr.shtns_l==i]), axis = 0)
        return
