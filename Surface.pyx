...
cdef class SurfaceBulkFormula(SurfaceBase):
    ...
    cpdef stats_io(self, DiagnosticVariables DV, PrognosticVariables PV, NetCDFIO_Stats Stats):
        Stats.write_surface_zonal_mean('zonal_mean_U_flux', DV.U.SurfaceFlux)
        Stats.write_surface_zonal_mean('zonal_mean_V_flux', DV.V.SurfaceFlux)
        Stats.write_surface_zonal_mean('zonal_mean_T_flux', PV.T.SurfaceFlux)
        Stats.write_surface_zonal_mean('zonal_mean_QT_flux', PV.QT.SurfaceFlux)
        return

    cpdef io(self, Parameters Pr, TimeStepping TS, DiagnosticVariables DV, PrognosticVariables PV, NetCDFIO_Stats Stats):
        Stats.write_2D_variable(Pr, TS.t,  'U_flux',  DV.U.SurfaceFlux)
        Stats.write_2D_variable(Pr, TS.t,  'V_flux',  DV.V.SurfaceFlux)
        Stats.write_2D_variable(Pr, TS.t,  'T_flux',  PV.T.SurfaceFlux)
        Stats.write_2D_variable(Pr, TS.t,  'QT_flux', PV.QT.SurfaceFlux)
        return
