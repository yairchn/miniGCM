...
cdef class HelzSuarezMoist(ForcingBase):
	...
	cpdef io(self, Parameters Pr, TimeStepping TS,  DiagnosticVariables DV, PrognosticVariables PV, NetCDFIO_Stats Stats):
		...
		Stats.write_3D_variable(Pr, int(TS.t), Pr.n_layers, 'T_eq', PV.T.forcing)
		return

	cpdef stats_io(self, DiagnosticVariables DV, PrognosticVariables PV, NetCDFIO_Stats Stats):
		Stats.write_zonal_mean('zonal_mean_T_eq', self.Tbar)
		Stats.write_zonal_mean('zonal_mean_T_eq', PV.T.forcing)
		Stats.write_meridional_mean('meridional_mean_T_eq', self.Tbar)
		Stats.write_meridional_mean('meridional_mean_T_eq', PV.T.forcing)
		return
