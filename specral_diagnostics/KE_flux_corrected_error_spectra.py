...
for Layer in np.arange(0,3):
    icount=0
    for it in time: Ek+=np.load(path+'Ek_flux_'+str(Layer)+'_0000000'+str(it).zfill(3)+'.npy'); icount+=1.;
    if Layer >0:
       for it in time: Ek-=np.load(path+'Ek_flux_error_'+str(Layer)+'_0000000'+str(it).zfill(3)+'.npy')
...
#ax.semilogx(ks,savgol_filter(Ek,5,1),'-k',linewidth=2,alpha=0.8,label='KE flux')
...
plt.tight_layout()
#plt.show()
plt.savefig('Kinetic_Energy_flux_corrected_error.pdf',dpi=150)
