...
path = '/home/scoty/miniGCM/Output.HeldSuarez.-correction2/Fields/'
    ...
    for Layer in np.arange(0,3):
        ...
        if (iplot_mean==1):
           if Layer==1 or Layer==2:
              ...
              print("zonal mean of ke-error minimum: ", np.amin(zonalmean), zonalmean.shape)
              maxAnomaly = np.amax(abs(zonalmean))
              plt.figure(59,figsize=(4,8))
              plt.clf()
           ...
           zonalmean=Wp.mean(axis=1)
           zonalstd=Wp.std(axis=1)
           #print("zonal mean of Wp: ", zonalmean, zonalmean.shape)
           maxAnomaly = np.amax(abs(zonalmean))
           plt.figure(53,figsize=(4,8))
           plt.clf()
           plt.title("Zonal means at day "+str(it))
           ...
           plt.plot(zonalmean-zonalstd,latDeg,'-k')
           plt.plot(zonalmean+zonalstd,latDeg,'-k')
           plt.savefig('Wp_zonalmean_'+str(Layer)+'_'+str(it).zfill(10)+'.png')
           np.save('Wp_zonalmean_'+str(Layer)+'_'+str(it).zfill(10),np.array(zonalmean))
