...
cdef extern from "diagnostic_variables.h":
    void diagnostic_variables(double cp, double lv, double Rd, double Rv, double Omega, double a,
           double* lat,  double* p,  double* E,  double* qt, double* ql, double* u,  double* v,
           double* div, double* ke, double* wp, double* gz, double* uv, double* TT, double* vT,
           double* M, double* T, Py_ssize_t k, Py_ssize_t imax, Py_ssize_t jmax, Py_ssize_t kmax) nogil
...
cdef class DiagnosticVariables:
    def __init__(self, Parameters Pr, Grid Gr):
        ...
        self.T  = DiagnosticVariable(Pr.nlats, Pr.nlons, Pr.n_layers,   Gr.SphericalGrid.nlm, 'air temperature', 'T','K' )
    ...
    cpdef io(self, Parameters Pr, TimeStepping TS, NetCDFIO_Stats Stats):
        ...
        Stats.write_3D_variable(Pr, int(TS.t), Pr.n_layers, 'T',             self.T.values)
    ...
    cpdef update(self, Parameters Pr, Grid Gr, PrognosticVariables PV):
        ...
        for k in range(nl):
            ...
            with nogil:
                diagnostic_variables(Pr.cp, Pr.Lv, Pr.Rd, Pr.Rv, Pr.Omega, Pr.rsphere, &Gr.lat[0,0], &PV.P.values[0,0,0], &PV.E.values[0,0,0],
                                     ...
                                     &self.M.values[0,0,0], &self.T.values[0,0,0], k, nx, ny, nl)
        return
