...
class ForcingNone():
	def __init__(self):
		return
	def initialize(self, Pr, Gr, PV, DV):
		return
...
class Forcing_HelzSuarez:
	...
	def update(self, Pr, Gr, TS, PV, DV, namelist):
		for k in range(Pr.n_layers):
			...
			self.Tbar_meridional=((Pr.T_equator-Pr.DT_y*np.sin(Gr.lat[:,0])**2-Pr.Dtheta_z*np.log(self.pressure_ratio_meridional)*np.cos(Gr.lat[:,0])**2)*(self.pressure_ratio_meridional)**Pr.kappa)
	...
	def io(self, Pr, Gr, TS, Stats):
		Stats.write_3D_variable(Gr, int(TS.t), Pr.n_layers, 'T_eq', self.Tbar)
		return
