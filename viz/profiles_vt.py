...
def main():
    ...
    var-= np.array(data.groups['zonal_mean'].variables['zonal_mean_V'])*np.array(data.groups['zonal_mean'].variables['zonal_mean_T'])
    t = np.divide(data.groups['zonal_mean'].variables['t'],3600.0*24.0)

    var2 = np.array(data2.groups['zonal_mean'].variables[varname])
    var2-= np.array(data2.groups['zonal_mean'].variables['zonal_mean_V'])*np.array(data2.groups['zonal_mean'].variables['zonal_mean_T'])
    ...
    for i in range(n):
        ...
        if i==0: plt.title("Eddy Heat Flux")
        if i==n-1:
            ax1.set_xlabel("$\overline{v'T'}$ / K m s$^{-1}$")
        plt.tight_layout()
    plt.savefig('vt_3layers_'+runname+'.png')
