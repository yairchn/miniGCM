...
def main():
    ...
    im1 = plt.plot(savgol_filter(np.mean(var[200:1000,:,0],axis=0)*100.,25,1),np.array(lat_list),'-k',linewidth=4,alpha=0.4,label='top')
    im2 = plt.plot(savgol_filter(np.mean(var[200:1000,:,1],axis=0)*100.,25,1),np.array(lat_list),'-k',linewidth=2,alpha=0.6,label='middle')
    im3 = plt.plot(savgol_filter(np.mean(var[200:1000,:,2],axis=0)*100.,25,1),np.array(lat_list),'-k',linewidth=1,alpha=0.8,label='bottom')
    plt.ylabel('Latitude / $\circ$')
    plt.grid(linestyle=':',alpha=0.6,linewidth=1)
    plt.title("Vertical Wind")
    #plt.xlim(-6,26)
    plt.xlabel("$\overline{\omega}$ / hPa s$^{-1}$")
    plt.legend()
    plt.tight_layout()
    plt.savefig('Wp_3layers_'+runname+'.png',dpi=150)
