...
path_restart='../Output.HeldSuarez.ReferenceRun/Fields_restart/'

#indices=np.arange(1,10).tolist()+np.arange(10,201,10).tolist()+np.arange(200,1001,40).tolist()
indices=np.arange(504,801,7).tolist()
#indices=np.arange(900,901,1).tolist()
for it in indices:
    ...
    for Layer in np.arange(0,3):
        ...
        T=netCDF4.Dataset(path+'Temperature_'+str(it*3600*24).zfill(8)+'.nc').variables['Temperature'][:,:,Layer]
        u-=netCDF4.Dataset(path_restart+'U_'+str(it*3600*24)+'.nc').variables['U'][:,:,Layer]
        v-=netCDF4.Dataset(path_restart+'V_'+str(it*3600*24)+'.nc').variables['V'][:,:,Layer]
        T-=netCDF4.Dataset(path_restart+'Temperature_'+str(it*3600*24).zfill(8)+'.nc').variables['Temperature'][:,:,Layer]
        ...
        if (iplot_ctr==1):
           ...
           plt.tight_layout()
           plt.savefig('v_diff_'+str(Layer)+'_'+str(it).zfill(10)+'.png')
           ...
           #plt.colorbar(orientation='horizontal',extend="both")
           plt.tight_layout()
           plt.savefig('u_diff_'+str(Layer)+'_'+str(it).zfill(10)+'.png')
           ...
           plt.colorbar(orientation='horizontal',extend="both")
           plt.tight_layout()
           plt.savefig('T_diff_'+str(Layer)+'_'+str(it).zfill(10)+'.png')
        ...
        if (iplot_spctr==1):
           ...
           np.save('EkTot_diff_'+str(Layer)+'_'+str(it).zfill(10)+'.npy',EkTot)
           np.save('EkRot_diff_'+str(Layer)+'_'+str(it).zfill(10)+'.npy',EkRot)
           np.save('EkDiv_diff_'+str(Layer)+'_'+str(it).zfill(10)+'.npy',EkDiv)
           np.save('ks.npy',ks)
        #
