...
def main():
    ...
    var-= np.array(data.groups['zonal_mean'].variables['zonal_mean_T'])*np.array(data.groups['zonal_mean'].variables['zonal_mean_T'])
    t = np.divide(data.groups['zonal_mean'].variables['t'],3600.0*24.0)

    var2 = np.array(data2.groups['zonal_mean'].variables[varname])
    var2-= np.array(data2.groups['zonal_mean'].variables['zonal_mean_T'])*np.array(data2.groups['zonal_mean'].variables['zonal_mean_T'])
    ...
    for i in range(n):
        ...
        if i==0: plt.title("Temperature Variance")
        if i==n-1:
            ax1.set_xlabel("$\overline{T'T'}$ / K$^{2}$")
        plt.tight_layout()
    plt.savefig('tt_3layers_'+runname+'.png')
