...
def main():
    ...
    for i in range(n):
        ...
        im1 = ax1.contourf(X,Y,np.fliplr(np.rot90(np.squeeze(var[:,:,i]), k=3)),levels=np.linspace(-40,40,20),extend='both',cmap='coolwarm')
        ...
        fig.colorbar(im1)
    plt.savefig('zonal_mean_VT_'+runname+'.png')
