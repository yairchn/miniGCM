...
cdef class LogFile:
    ...
    cpdef update(self, Parameters Pr, TimeStepping TS, DiagnosticVariables DV, PrognosticVariables PV, wallclocktime):
        ...
        for i in range(Pr.n_layers):
            os.system('echo T layer '+str(i+1)+' min max ' + str(DV.T.values.base[:,:,i].min())
                      + ' ' + str(DV.T.values.base[:,:,i].max()) + '>> '+Pr.logfilename)

        for i in range(Pr.n_layers):
            if Pr.moist_index > 0.0:
                os.system('echo QT layer '+str(i+1)+' min max ' + str(PV.QT.values.base[:,:,i].min())
                          + ' ' + str(PV.QT.values.base[:,:,i].max()) + '>> '+Pr.logfilename)
                os.system('echo dTdt_mp layer '+str(i+1)+' min max ' + str(DV.T.mp_tendency.base[:,:,i].min())
                          + ' ' + str(DV.T.mp_tendency.base[:,:,i].max()) + '>> '+Pr.logfilename)
        return