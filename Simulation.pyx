...
class Simulation:
    ...
    def run(self, namelist):
        ...
        while self.TS.t <= self.TS.t_max:
            ...
            print('.PV.reset_pressures_and_bcs',time.time() - t0)
            t0 = time.time()
            self.PV.spectral_to_physical(self.Pr, self.Gr)
            print('.PV.spectral_to_physical',time.time() - t0)
            t0 = time.time()
            self.DV.update(self.Pr, self.Gr, self.PV)
            print('.DV.update',time.time() - t0)
            t0 = time.time()
            self.DV.physical_to_spectral(self.Pr, self.Gr)
            print('.DV.physical_to_spectral',time.time() - t0)
            t0 = time.time()
            self.Case.update(self.Pr, self.Gr, self.PV, self.DV, self.TS)
            print('.Case.update',time.time() - t0)
            t0 = time.time()
