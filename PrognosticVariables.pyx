...
cdef extern from "tendency_functions.h":
    void rhs_q(double* p, double* qt, double* u, double* v, double* wp,
    ...
    void rhs_U(double cp, double* p, double* gz, double* T, double* u, double* v, double* wp,
...
cdef class PrognosticVariables:
    def __init__(self, Parameters Pr, Grid Gr, namelist):
        self.U   = PrognosticVariable(Pr.nlats, Pr.nlons, Pr.n_layers,  Gr.SphericalGrid.nlm,'zonal mean U' ,'U' ,'m/s')
        self.q   = PrognosticVariable(Pr.nlats, Pr.nlons, Pr.n_layers,  Gr.SphericalGrid.nlm,'perturbation PV' ,'q','1/(ms)')
        return
    ...
    cpdef initialize_io(self, Parameters Pr, NetCDFIO_Stats Stats):
        Stats.add_zonal_mean('zonal_mean_preturbation_PV')
        Stats.add_zonal_mean('zonal_mean_U')
        Stats.add_meridional_mean('meridional_mean_preturbation_PV')
    ...
    cpdef physical_to_spectral(self, Parameters Pr, Grid Gr):
        ...
        for k in range(nl):
            ...
            self.q.spectral.base[:,k] = Gr.SphericalGrid.grdtospec(self.q.values.base[:,:,k])
        return
    ...
    cpdef spectral_to_physical(self, Parameters Pr, Grid Gr):
        ...
        for k in range(nl):
            self.U.values.base[:,:,k]  = Gr.SphericalGrid.spectogrd(self.U.spectral.base[:,k])
            self.q.values.base[:,:,k] = Gr.SphericalGrid.spectogrd(self.q.spectral.base[:,k])
        return

    # quick utility to set arrays with values in the "new" arrays
    cpdef set_old_with_now(self):
        self.U.old  = self.U.now.copy()
        self.q.old = self.q.now.copy()
        return

    cpdef set_now_with_tendencies(self):
        self.U.now  = self.U.tendency.copy()
        self.q.now = self.q.tendency.copy()
        return
    ...
    cpdef stats_io(self, Parameters Pr, NetCDFIO_Stats Stats):
        ...
        Stats.write_zonal_mean('zonal_mean_preturbation_PV',self.q.values)
        Stats.write_zonal_mean('zonal_mean_U',self.U.values)
        Stats.write_meridional_mean('meridional_mean_preturbation_PV',self.q.values)
    ...
    cpdef io(self, Parameters Pr, Grid Gr, TimeStepping TS, NetCDFIO_Stats Stats):
        ...
        Stats.write_3D_variable(Pr, int(TS.t),nl, 'zonal mean U', self.U.values)
        Stats.write_3D_variable(Pr, int(TS.t),nl, 'perturbation PV',   self.q.values)
        return
    ...
    cpdef compute_tendencies(self, Parameters Pr, Grid Gr, PrognosticVariables PV, DiagnosticVariables DV):
        ...
        for k in range(nl):
            ...
            with nogil:
                rhs_U(&PV.P.values[0,0,0], &DV.gZ.values[0,0,0], &PV.Vorticity.values[0,0,0],
                ...
                rhs_q(Pr.cp, &PV.P.values[0,0,0], &DV.gZ.values[0,0,0], &PV.T.values[0,0,0], &DV.U.values[0,0,0],
            ...
            with nogil:
                for i in range(nlm):
                    PV.U.tendency[i,k]  = (Vort_forc[i] - Divergent_momentum_flux[i]- w_vort_up[i] - w_vort_dn[i]
                                                     + Vort_sur_flux[i] + PV.Vorticity.ConvectiveFlux[i,k] + PV.Vorticity.sp_forcing[i,k])
                    PV.q.tendency[i,k] = (Vortical_momentum_flux[i] - Dry_Energy_laplacian[i]- w_div_up[i] - w_div_dn[i]
