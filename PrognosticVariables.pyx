...
cdef class PrognosticVariable:
    ...
    cpdef set_bcs(self, Parameters Pr, Grid Gr):
        ...
        for i in range(ng):
            ...
            self.values.base[i,:,:]       = self.values.base[2*ng-i,:,:]
            self.values.base[nx-1+i,:,:]    = self.values.base[2*ng+nx-1-i,:,:]
            self.values.base[:,i,:]       = self.values.base[:,2*ng-i,:]
            self.values.base[:,ny-1+i,:]    = self.values.base[:,2*ng+ny-1-i,:]
        return
...
cdef class PrognosticVariables:
    def __init__(self, Parameters Pr, Grid Gr, namelist):
        cdef:
            Py_ssize_t ng = Gr.ng
        self.U  = PrognosticVariable(Gr.nx+2*ng, Gr.ny+2*ng, Gr.nl, 'zonal velocity'      ,'u'  ,'1/s')
        self.V  = PrognosticVariable(Gr.nx+2*ng, Gr.ny+2*ng, Gr.nl, 'meridionla velocity' ,'v'  ,'1/s')
    ...
    cpdef compute_tendencies(self, Parameters Pr, Grid Gr, PrognosticVariables PV, DiagnosticVariables DV):
        cdef:
            ...
            double [:,:,:] duu_dx = np.zeros((nx+2*ng, ny+2*ng, nl),dtype=np.float64, order='c')
            double [:,:,:] dvu_dy = np.zeros((nx+2*ng, ny+2*ng, nl),dtype=np.float64, order='c')
            double [:,:,:] duv_dx = np.zeros((nx+2*ng, ny+2*ng, nl),dtype=np.float64, order='c')
            double [:,:,:] dvv_dy = np.zeros((nx+2*ng, ny+2*ng, nl),dtype=np.float64, order='c')
        ...
        for i in range(ng,nx+ng):
            for j in range(ng,ny+ng):
                for k in range(nl):
                    duHdx = 0.5*(PV.U.values[i+1,j,k] * PV.H.values[i+1,j,k] -
                                 PV.U.values[i-1,j,k] * PV.H.values[i-1,j,k])*dxi
