...
cdef class PrognosticVariables:
    def __init__(self, Gr):
        self.Vorticity   = PrognosticVariable(Gr.nlats, Gr.nlons, Gr.n_layers,  Spharmt.nlm,'Vorticity' , 'zeta',  '1/s' )
        self.Divergence  = PrognosticVariable(Gr.nlats, Gr.nlons, Gr.n_layers,  Spharmt.nlm,'Divergance', 'delta', '1/s' )
        self.T           = PrognosticVariable(Gr.nlats, Gr.nlons, Gr.n_layers,  Spharmt.nlm,'Temperature'       ,  'T','K' )
        self.QT          = PrognosticVariable(Gr.nlats, Gr.nlons, Gr.n_layers,  Spharmt.nlm,'Specific_humidity' ,  'qt','kg/kg' )
        self.P           = PrognosticVariable(Gr.nlats, Gr.nlons, Gr.n_layers+1,Spharmt.nlm,'Pressure'          ,  'p','pasc' )
        return
    ...
    cpdef initialize(self, Grid Gr, DiagnosticVariables DV):
        ...
        for k in range(Gr.n_layers):
            self.P.spectral[:,k]           = Spharmt.grdtospec(self.P.values[:,:,k])
            self.T.spectral[:,k]           = Spharmt.grdtospec(self.T.values[:,:,k])
            self.QT.spectral[:,k]          = Spharmt.grdtospec(self.QT.values[:,:,k])
            self.Vorticity.spectral[:,k]   = Spharmt.grdtospec(self.Vorticity.values[:,:,k])
            self.Divergence.spectral[:,k]  = Spharmt.grdtospec(self.Divergence.values[:,:,k])
        self.P.spectral[:,Gr.n_layers]     = Spharmt.grdtospec(self.P.values[:,:,Gr.n_layers])
    ...
    cpdef spectral_to_physical(self, Grid Gr):
        cdef:
            Py_ssize_t k
        for k in range(Gr.n_layers):
            self.Vorticity.values[:,:,k]  = Spharmt.spectogrd(self.Vorticity.spectral[:,k])
            self.Divergence.values[:,:,k] = Spharmt.spectogrd(self.Divergence.spectral[:,k])
            self.P.values[:,:,k]          = Spharmt.spectogrd(self.P.spectral[:,k])
            self.T.values[:,:,k]          = Spharmt.spectogrd(self.T.spectral[:,k])
            self.QT.values[:,:,k]         = Spharmt.spectogrd(self.QT.spectral[:,k])
        # I am updating only the surface pressure 
        self.P.values[:,:,Gr.n_layers] = Spharmt.spectogrd(self.P.spectral[:,Gr.n_layers])
        return
    ...
    cpdef compute_tendencies(self, Grid Gr, PrognosticVariables PV, DiagnosticVariables DV, namelist):
        ...
        Vortical_P_flux, Divergent_P_flux = Spharmt.getvrtdivspec(np.multiply(DV.U.values[:,:,nz-1],np.subtract(PV.P.values[:,:,nz-1],PV.P.values[:,:,nz])),
        ...
        for k in range(nz):
            ...
            Vortical_momentum_flux, Divergent_momentum_flux = Spharmt.getvrtdivspec(u_vertical_flux, v_vertical_flux)
        ...
        for k in range(nz):
            ...
            dp_ratio = Spharmt.grdtospec(dp_ratio_)
            # compute he energy laplacian for the divergence equation
            Dry_Energy_laplacian = Spharmt.lap*Spharmt.grdtospec(
            ...
            Vorticity_flux, Divergence_flux = Spharmt.getvrtdivspec(np.multiply(DV.U.values[:,:,k],np.add(PV.Vorticity.values[:,:,k],Gr.Coriolis)),
                                                  np.multiply(DV.V.values[:,:,k],np.add(PV.Vorticity.values[:,:,k],Gr.Coriolis)))
            Vortical_T_flux, Divergent_T_flux = Spharmt.getvrtdivspec(np.multiply(DV.U.values[:,:,k],PV.T.values[:,:,k]),
                                                  np.multiply(DV.V.values[:,:,k],PV.T.values[:,:,k])) # Vortical_T_flux is not used
            Vortical_QT_flux, Divergent_QT_flux = Spharmt.getvrtdivspec(np.multiply(DV.U.values[:,:,k],PV.QT.values[:,:,k]),
            ...
            dp = np.subtract(PV.P.values[:,:,k+1],PV.P.values[:,:,k])
            Thermal_expension = Spharmt.grdtospec(np.multiply(np.divide(DV.Wp.values[:,:,k+1],Gr.cp),np.divide(dgZ/dp)))
            # compute tendencies
            ...
            PV.T.tendency[:,k] = (- Divergent_T_flux + Spharmt.grdtospec(-PV.T.VerticalFlux[:,:,k] + np.multiply(PV.T.VerticalFlux[:,:,k-1],dp_ratio_))
                                  - Thermal_expension  + PV.T.forcing[:,k])
            # PV.QT.tendency[:,k] = - Divergent_QT_flux + Spharmt.grdtospec(-PV.QT.VerticalFlux[:,:,k] + np.multiply(PV.QT.VerticalFlux[:,:,k-1],dp_ratio_)) + PV.QT.forcing[:,k]
